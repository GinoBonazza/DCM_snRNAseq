...
FB[[i]] <- SCTransform(FB[[i]], vst.flavor = "v2", vars.to.regress = c("percent.mt"), return.only.var.genes = TRUE, variable.features.n = 3000, min_cells = 0)
}
```
...
```{r PCA}
features <- SelectIntegrationFeatures(object.list = FB, nfeatures = 3000)
...
VlnPlot_scCustom(FB, features = c("RYR2", "TTN", "VWF", "scDblFinder.score"), assay="RNA", num_columns = 2, pt.size = 0.01, colors_use = cell_type_cols) + theme(axis.title.x = element_blank())
```
...
FB@meta.data[FB@meta.data$SCT_snn_res.0.3 %in% c("5"), "cell_state"] = "FB6"

FB$cell_state <- factor(FB$cell_state, levels = c("FB1", "FB2", "FB3","FB4","FB5", "FB6"))
...
VlnPlot_scCustom(FB, features = c("ACSM3", "PCOLCE2", "KAZN", "FGF14", "GPC6", "NFATC2"), num_columns = 6, colors_use = cell_type_cols, pt.size = 0)
```
...
```{r FeatPlot_markers_myoFB, fig.width=16, fig.height=3}
...
dfsample$FB6 = bitr(dfsample$FB6, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
