...
clusters <- c("Cardiomyocytes", "Endothelial cells", "Fibroblasts",
              ...
              "T lymphocytes", "Neuronal cells", "Endocardial cells"
              )
for (j in 1:length(clusters)) {
  new_dir <- here::here(output_dir_data, clusters[j])
  if (!dir.exists(new_dir)) dir.create(new_dir)
}

for (j in 1:length(clusters)) {
  cat("  \n<h2>",  clusters[j], "</h2>\n")
  setwd(here::here(output_dir_data, clusters[j]))
  ...
  mds_plot <- ggplot(mds_df, aes(x, y, col = group_id)) + 
    ...
    theme(panel.grid.minor = element_blank(),
          plot.title = element_text(hjust = 0.5)) +
    coord_fixed() + 
    guides(color = guide_colorbar(title = colnames(metadata[4])))
  ggsave(plot = mds_plot, filename = paste0(clusters[j], "_MDSplot_", colnames(metadata[4]), ".png"), 
          width = 5, height = 3, dpi=300, bg = "white")
  cat("![](",paste0(getwd(), "/", clusters[j], "_MDSplot_", colnames(metadata[4]), ".png"),"){width=40%}")
  
  rm(pseudocounts, mds, mds_df, keep.samples, keep.genes, metadata_subset, sample.formula, mds_plot)
  ...
  GOclusterplot <- compareCluster(geneCluster = genelist, fun = "enrichGO", OrgDb = "org.Hs.eg.db", universe = rownames(data))
  if (!is.null(GOclusterplot)) {
    ...
    if (!is.null(GOclusterplot) && nrow(GOclusterplot@compareClusterResult) > 0) {
      ...
      GOclusterplot <- setReadable(GOclusterplot , 'org.Hs.eg.db', 'SYMBOL')
  ...
  KEGGclusterplot <- compareCluster(geneCluster = genelist, fun = "enrichKEGG", organism = 'hsa', universe = rownames(data))
  if (!is.null(KEGGclusterplot)) {
    ...
    if (!is.null(KEGGclusterplot) && nrow(KEGGclusterplot@compareClusterResult) > 0) {
      ...
      KEGGclusterplot <- setReadable(KEGGclusterplot , 'org.Hs.eg.db', 'SYMBOL')
  ...
  REACTOMEclusterplot <- compareCluster(geneCluster = genelist, fun = "enrichPathway", organism = "human", universe = rownames(data))
  if (!is.null(REACTOMEclusterplot)) {
    REACTOMEclusterplot <- gsfilter(REACTOMEclusterplot, by = 'Count', min = 3)
    if (!is.null(REACTOMEclusterplot) && nrow(REACTOMEclusterplot@compareClusterResult) > 0) {
      pathways <- dotplot(REACTOMEclusterplot, showCategory = 10) + scale_fill_gradient(low = "red", high = "grey") +
        theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 15), 
              plot.title = element_text(hjust = 0.5, size = 16),
              axis.text.y = element_text(size = 13), axis.title.x = element_blank()) +
        ggtitle(paste0(clusters[j], "\nReactome enrichment analysis"))
      ggsave(plot = pathways, filename = paste0(clusters[j], "_Pathways_", colnames(metadata)[i], "_Reactome.png"), 
             width = 6, height = 10, dpi=300, bg = "white")
      cat("![](",paste0(getwd(), "/", clusters[j], "_Pathways_", colnames(metadata)[i], "_Reactome.png"),"){width=40%}")
      REACTOMEclusterplot <- setReadable(REACTOMEclusterplot , 'org.Hs.eg.db', 'SYMBOL')
      REACTOME_df <- as.data.frame(REACTOMEclusterplot@compareClusterResult)
      REACTOME_df$geneID <- gsub("/", ", ", REACTOME_df$geneID)
      write.xlsx(REACTOME_df, file = paste0(clusters[j], "_Pathways_", colnames(metadata)[i], "_REACTOME", ".xlsx"), 
                 row.names = F)
    }
  }
  }
