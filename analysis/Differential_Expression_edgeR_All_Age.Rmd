---
title: "Differential Espression of all cell types and all clinical parameters (plus age as a covariate) using edgeR"
...
current_file <- "Differential_expression_edgeR_All_Age"
...
for (j in 1:length(clusters)) {
  ...
  mds_plot <- ggplot(mds_df, aes(x, y, col = group_id)) + 
    ...
    theme(panel.grid.minor = element_blank(),
          plot.title = element_text(hjust = 0.5)) +
    coord_fixed() + 
    guides(color = guide_legend(title = colnames(metadata[4])))
  ...
  for (i in 5:(length(metadata))) {
    ...
    colnames(design)[3] <- colnames(metadata_subset["Age_years"])
