...
DefaultAssay(CM) <- "SCT"
CM <- RunUMAP(CM, dims = 1:50, reduction = "harmony")
CM <- FindNeighbors(CM, dims = 1:50, reduction = "harmony")
...
add_metadata <- left_join(CM[["Sample"]], Clinical_data)
row.names(add_metadata) <- row.names(CM[[]])
CM <- AddMetaData(CM, metadata = add_metadata)
...
Markers <- FindAllMarkers(CM, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
...
VlnPlot_scCustom(CM, features = c("PLCG2", "LINC02388", "NRAP", "ARHGAP24", "IGF1R"), num_columns = 5, colors_use = cell_type_cols)
```

```{r FeatPlot_markers_CM, fig.width=15, fig.height=3}
FeaturePlot_scCustom(CM, features = c("PLCG2", "LINC02388", "NRAP", "ARHGAP24", "IGF1R"), num_columns = 5)
```
...
```{r Barplot_CM_Group, fig.height=6, fig.width=5, message=FALSE}
...
```{r TF_Feat_STAT2, fig.width=12, fig.height=4}
...
p2 <- Seurat::FeaturePlot(CM, features = c("STAT2"), pt.size = 0.1) + 
      ggplot2::scale_colour_gradient2(low = TF_cols[1], high = TF_cols[101], mid = "white") +
      ggplot2::ggtitle('STAT2 activity')

DefaultAssay(object = CM) <- "RNA"
p3 <- Seurat::FeaturePlot(CM, 
                          features = c("STAT2"), pt.size = 0.1) + 
      ggplot2::ggtitle('STAT2 expression')
...
```{r Pathway_Feat_WNT, fig.width=8, fig.height=4}
Seurat::DefaultAssay(object = CM) <- "pathwaysmlm"
...
p2 <- Seurat::FeaturePlot(CM, features = c("WNT"), 
                      pt.size = 0.1) + 
      ggplot2::scale_colour_gradientn(colors = Pathway_cols
                                      ) +
      ggplot2::ggtitle('WNT activity')
