...
```{r QC_VlnPlots_1b, fig.width=12, fig.height=3, dpi = 300}
VlnPlot_scCustom(CM, features = c("nCount_RNA", "nFeature_RNA", "percent.mt"), num_columns = 3, pt.size = 0, colors_use = cell_type_cols) + theme(axis.title.x = element_blank())
```
...
```{r DotPlot_markers, fig.width=6.5, fig.height=4, dpi = 300}
Idents(CM) <- factor(
  Idents(CM),
...
```{r CM_cell_states.csv, eval=TRUE}
...
CM_counts <- as.data.frame(props$Counts) %>%   
  pivot_wider(
    names_from  = clusters,  
    values_from = Freq       
  )

supp_data_file <- here::here(output_dir_data, "CM_subclustering_SuppData.xlsx")
if (file.exists(supp_data_file)) file.remove(supp_data_file)

writexl::write_xlsx(
  list(
    Markers_all = Markers,
    Cell_counts = CM_counts,
    Cell_proportions = CM_proportions,
